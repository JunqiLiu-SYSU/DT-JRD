...
png_paths = os.path.join(data_path, 'png')
png_names = list(natsorted(os.listdir(png_paths)))
png_names = [png_name[:5] for png_name in png_names]
Combination = []

for QP in QPs:
    for png_name in tqdm(png_names):
        png_path = os.path.join(png_paths, png_name + '.png')
        ...
        resize_decode_png_path = 'C:/Users/Administrator.DESKTOP-4KE2HTR/Desktop/Codec/All-intra-codec/test_img/allintraQP/' + str(QP) + '/resize_decode_png/' + png_name + '.png'
        Combination.append([width, height, decode_png_path, resize_decode_png_path])

def decode(Combination):
    width = Combination[0]
    height = Combination[1]
    decode_png_path = Combination[2]
    resize_decode_png_path = Combination[3]
    cmd = "ffmpeg -i " + decode_png_path + " -s " + str(width) + 'x' + str(height) + ' ' + resize_decode_png_path
    print(cmd)
    os.system(cmd)
